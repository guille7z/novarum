...
export default function OTPEmail({ otp, intent }: { otp: number; intent: 'verify-email' | 'reset-password' }) {
    return (
        <Tailwind>
            <Section className="flex justify-center items-center w-full min-h-screen font-sans">
                <Section className="flex flex-col items-center w-76 rounded-2xl px-6 py-1 bg-mauve-50">
                    <Text className="text-xs font-medium text-violet-500">
                      {intent === 'verify-email' ? 'verify your email address' : 'reset your password'}
                    </Text>
